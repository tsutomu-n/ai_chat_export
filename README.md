...
3. チャットページ（例: `grok.com` / `chatgpt.com`）でそのブックマークをクリックし、`開始`

## 既存ブックマークの更新
1. `ai-chat-export.oneliner.js` の中身を全部コピー
2. 既存ブックマークを編集
3. URL欄を丸ごと置き換えて保存
...
4. 結果ダイアログでクリップボード保存またはファイル保存を選ぶ
...
- `ai-chat-export.min.js`
  - lint / quick-check 用の minified JS
...
## Note
- `ai-chat-export.oneliner.js` is a `javascript:` URL string, not a normal JavaScript source file.
- Run lint / syntax checks against `ai-chat-export.js` or `ai-chat-export.min.js`.
- `.eslintignore` and `.vscode/settings.json` exclude generated files and history directories from lint / search / file watching.
